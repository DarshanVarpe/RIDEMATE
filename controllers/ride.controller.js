...
module.exports.createRide = async (req, res, next) => {
    ...
    try {
        ...
        const vehicle = await Vehicle.findOne({ _id: vehicleDetails, user: req.user.id, deleted: false });
        if (!vehicle) {
            return res.status(400).json({
                message: "Unauthorized: Invalid or deleted vehicle"
            });
        }

        if (vehicle.vehicleType === 'bike' && seats > 1) {
            return res.status(400).json({
                message: "A bike cannot offer more than 1 seat"
            });
        }
...
module.exports.getAvailableRides = async (filters) => {
    try {
        ...
        if (filters.from) query.from = new RegExp(escapeRegex(filters.from), 'i'); // Case-insensitive search
        if (filters.to) query.to = new RegExp(escapeRegex(filters.to), 'i'); // Case-insensitive search
...
module.exports.joinRide = async (req, res, next) => {
    ...
    try {
        ...
        const activeRide = await rideModel.findOne({
            $or: [
                { passengers: userId, status: { $in: ["pending", "progress"] } },
                { driver: userId, status: { $in: ["pending", "progress"] } }
            ]
        });
        ...
        const notificationMessage = `A new passenger has joined your ride from ${ride.from} to ${ride.to}.`;
...
module.exports.cancelBooking = async (req, res, next) => {
    ...
    try {
        let ride = await rideModel.findOne({ _id: rideId, status: "pending" });
        
        if (!ride) {
            // Check if it's in progress
            const inProgressRide = await rideModel.findOne({ _id: rideId, status: "progress" });
            if (inProgressRide) {
                return res.status(400).json({ error: 'You cannot cancel a booking for a ride that is already in progress.' });
            }
