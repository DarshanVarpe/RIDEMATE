...
module.exports.register = async (req, res, next) => {
    ...
    const lowCasePlate = numberPlate.toLowerCase().replace(/[\s-]/g, '');
    try {
...
module.exports.deleteVehicles = async (req, res, next) => {
    ...
    const normalizedPlate = numberPlate.toLowerCase().replace(/[\s-]/g, '');

    try {
        const vehicle = await vehicleModel.findOne({ user: req.user.id, numberPlate: normalizedPlate });
