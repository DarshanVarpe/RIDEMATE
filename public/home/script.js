...
function manageRideDetailsTempleteForPassenger(ride) {
    ...
    return `
        ...
        <div class="relative sm:w-1/2 sm:h-full sm:rounded-l-lg flex-shrink-0">
            ...
            <!-- Placeholder Map -->
            <div class="w-full h-44 sm:h-full bg-slate-50 flex flex-col justify-center items-center text-center p-4 rounded-t-lg sm:rounded-t-none sm:rounded-l-lg border-b sm:border-b-0 sm:border-r border-gray-200">
                <div class="text-[#6d74fc] text-xl font-bold mb-3">📍 Route Preview</div>
                <div class="text-sm text-gray-500 mb-1">Pickup:</div>
                <div class="text-sm font-bold text-gray-800 mb-3">${ride.from}</div>
                <div class="text-sm text-gray-500 mb-1">Destination:</div>
                <div class="text-sm font-bold text-gray-800 mb-4">${ride.to}</div>
                <div class="text-xs text-gray-400 italic">(Interactive map coming soon)</div>
            </div>        </div>
...
function openWhatsApp(phone) {
    window.open(`https://wa.me/92${phone}?text=Hello%20from%20RideMate`, '_blank');
}
    ...
    fetch('/ride/cancelBooking', {
        ...
        if (!response.ok) {
            return response.json().then(errorData => {
                alert(errorData.error || errorData.message || 'Something went wrong');
                closeModal();
